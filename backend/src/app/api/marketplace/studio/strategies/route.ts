  ...
  try {
    ...
    void queueAdminNotification({
      ...
      text:
        ...
        `Strategy: ${formatAdminStrategyLine({ id, name: newStrategy.name, type: "algo", symbol: newStrategy.symbol })}\n` +
        `Creator: ${formatAdminUserLine(session.user)}\n\n` +
        `Status: <code>draft</code>`,
      meta: { strategyId: id, type: "algo", creatorId: session.user.id, status: "draft" },
    });
